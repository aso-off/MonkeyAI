...
def _moderation_keyboard(lang: str) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=toggle_text, callback_data="toggle_moderation", style="primary", icon_custom_emoji_id="6030657343744644592")],
        [InlineKeyboardButton(text=t("back_to_admin", lang), callback_data="admin_panel", icon_custom_emoji_id="5960671702059848143")],
    ])


def _moderation_text(lang: str) -> str:
    status = t("moderation_status_enabled", lang) if settings.enable_content_moderation else t("moderation_status_disabled", lang)
    return (
        f"{t('moderation_settings_title', lang)}\n\n"
        f"{t('current_status', lang)} {status}\n\n"
        f"{t('moderation_settings_help', lang)}"
    )
