...
def _admin_md(lang: str) -> str:
    head, _, rest = t("admin_welcome", lang).partition("\n")
    return rp.join(rp.heading(head, 1), rest)
        ...
        inline_keyboard=[
            ...
            [
                ...
                InlineKeyboardButton(
                    ...
                    style="primary",
                    icon_custom_emoji_id="6044356915029348425",
                ),
            ],
            [
                InlineKeyboardButton(
                    text=t("moderation", lang),
                    callback_data="admin_moderation",
                    style="primary",
                    icon_custom_emoji_id="6032850693348399258",
                )
            ],
            [
                InlineKeyboardButton(
                    text=t("whitelist_management", lang),
...
async def cmd_admin(message: Message, language: str, db_user=None) -> None:
    ...
    if not is_admin:
        return
    await rp.answer_panel(message, _admin_md(language), reply_markup=_admin_panel_keyboard(language))
...
async def cb_admin_panel(query: CallbackQuery, language: str, db_user=None) -> None:
    ...
    if not isinstance(query.message, Message):
        return
    await rp.edit_panel(query.message, _admin_md(language), reply_markup=_admin_panel_keyboard(language))
