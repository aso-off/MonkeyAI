<template>
  <div class="flex">
    ...
    <div
      id="sidebar"
      :class="['sidebar__wrapper', { 'sidebar__wrapper-active': sidebarActive }]"
    ...
    <div class="sidebar">
        <div class="sidebar__user">
          <div class="sidebar__user-avatar"><img :src="model_logo" alt=""></div>
          ...
          <button class="sidebar__item" @click="clearCurrentChat">
            <div class="sidebar__item-icon">
              ...
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                <path d="M8 12H12M12 12H16M12 12V16M12 12V8M12 21C7.02944 21 3 16.9706 3 12C3 7.02944 7.02944 3 12 3C16.9706 3 21 7.02944 21 12C21 16.9706 16.9706 21 12 21Z" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
              </svg>
            </div>
    ...
    <header>
      <div class="header__sidebar" @click="toggleSidebar">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
          ...
          <path d="M3 8.5H21" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> <!-- Изменено stroke на currentColor -->
          <path d="M3 15.5H16" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> <!-- Изменено stroke на currentColor -->
        </svg>
      </div>

      <!-- Обертка для центрирования -->
      <div class="header__model-wrapper">
        <div class="header__model" ref="modelSelector" @click="toggleModelDropdown">
          <div class="header__model-name" v-html="selectedModel"></div>
          <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
               xmlns="http://www.w3.org/2000/svg">
            <path d="M16 10L12 14L8 10" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> <!-- Изменено stroke на currentColor -->
          </svg>
        </div>
      </div>

      <div class="header__limits">
      </div>
    </header>
    ...
    <div class="chat-wrapper">
      <div class="chat-content"
        ...
        <div v-if="hasMoreToLoad" class="load-more-row">
          ...
          >
            {{ isLoadingOlderMessages ? $t('load_more_loading') : $t('load_more_messages') }}
          </button>
        </div>
          ...
          >
            ...
            <div v-else>
              ...
              <div v-else-if="msg.type === 'bot'">
                ...
                <div v-if="index === streamingBotIdx && !msg.text" class="ai-thinking">
                ...
                <div v-if="index !== streamingBotIdx && msg.text" class="copy-wrapper">
                  <!-- Копировать -->
                  <div class="btn-tip-wrap" :data-tip="copiedIndex === index ? t('tooltip_copied') : t('tooltip_copy')">
                    <button
                      class="copy-button"
                      :class="{ 'copied-state': copiedIndex === index }"
                      @click="copyToClipboard(msg.text, index)"
                    >
                      <svg v-if="copiedIndex === index" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 50 50">
                  ...
                  <div class="btn-tip-wrap" :data-tip="t('tooltip_like')">
                    <button
                      class="action-btn"
                      :class="{ 'reacted-like': reactionMap.get(index) === 'like' }"
                      @click="onReaction(index, 'like', chatMessages[index - 1]?.text ?? '', msg.text)"
                    >
                      <svg viewBox="0 -960 960 960" fill="currentColor" width="16" height="16"><path d="M720-120H280v-520l280-280 50 50q7 7 11.5 19t4.5 23v14l-44 174h258q32 0 56 24t24 56v80q0 7-2 15t-4 15L794-168q-9 20-30 34t-44 14Zm-360-80h360l120-280v-80H480l54-220-174 174v406Zm0-406v406-406Zm-80-34v80H160v360h120v80H80v-520h200Z"/></svg>
                    </button>
                  </div>
                  <!-- Дизлайк -->
                  <div class="btn-tip-wrap" :data-tip="t('tooltip_dislike')">
                    <button
                      class="action-btn"
                      :class="{ 'reacted-dislike': reactionMap.get(index) === 'dislike' }"
                      @click="onReaction(index, 'dislike', chatMessages[index - 1]?.text ?? '', msg.text)"
                    >
                      <svg viewBox="0 -960 960 960" fill="currentColor" width="16" height="16" style="transform:scaleX(-1)"><path d="M240-840h440v520L400-40l-50-50q-7-7-11.5-19t-4.5-23v-14l44-174H120q-32 0-56-24t-24-56v-80q0-7 2-15t4-15l120-282q9-20 30-34t44-14Zm360 80H240L120-480v80h360l-54 220 174-174v-406Zm0 406v-406 406Zm80 34v-80h120v-360H680v-80h200v520H680Z"/></svg>
                    </button>
                  </div>
...
function toggleSidebar() {
  if (sidebarActive.value) {
    ...
    setTimeout(() => { sidebarDisplay.value = false; }, 200);
  } else {
    sidebarDisplay.value = true;
    setTimeout(() => { sidebarActive.value = true; }, 10);
  }
}
...
const selectedModel = ref(models.value.find(m => m.id === currentModelId.value)?.name ?? models.value[0].name);
...
const isSubmitDisabled = computed(() => messageText.value.trim() === '' || isStreaming.value);
...
async function loadOlderMessages() {
  if (!hasMoreToLoad.value || isLoadingOlderMessages.value || !store.dialogId) return;
...
async function sendMessage() {
  ...
  generationWatchdog = setInterval(() => {
    if (streamingBotIdx.value === -1) {
      clearInterval(generationWatchdog!); generationWatchdog = null; return;
    }
  ...
  try {
    if (isImageModel) {
      ...
      if (result.is_flagged) {
        chatMessages.value[botIdx] = { type: 'bot', contentType: 'text', text: t('message_flagged') };
      } else {
        chatMessages.value[botIdx] = { type: 'bot', contentType: 'text', text: result.answer };
      }
    }
  } catch (e) {
  ...
  if (generationWatchdog) { clearInterval(generationWatchdog); generationWatchdog = null; }
  await nextTick();
    ...
    if (ih <= ph - m * 2) {
      ...
      while (offsetPx < canvas.height) {
        ...
        sliceCanvas.getContext('2d')!.drawImage(
          canvas, 0, offsetPx, canvas.width, sliceH, 0, 0, canvas.width, sliceH,
        );
        const sliceIh = (sliceH / canvas.width) * iw;
        pdf.addImage(sliceCanvas.toDataURL('image/png'), 'PNG', m, m, iw, sliceIh);
        offsetPx += sliceH;
...
onMounted(async () => {
  ...
  if (footerEl) {
    const updateFooterHeight = () => {
      ...
      const atBottom = el ? (el.scrollHeight - el.scrollTop - el.clientHeight) < 4 : false;
  ...
  wsClient.setTypeHandler('chat_error', (msg) => {
    ...
    if (botIdx !== undefined) {
      chatMessages.value[botIdx] = {
        type: 'bot', contentType: 'text',
        text: t('error_response') + ': ' + ((msg.error as string) || 'chat error'),
      };
    }
    store.setChatHistory(chatMessages.value);
  });
  ...
  wsClient.setTypeHandler('image_error', (msg) => {
    ...
    if (botIdx !== undefined) {
      chatMessages.value[botIdx] = {
        type: 'bot', contentType: 'text',
        text: t('error_response') + ': ' + ((msg.error as string) || 'image error'),
      };
    }
    store.setChatHistory(chatMessages.value);
  });
  ...
  wsClient.setTypeHandler('connection_ack', async (msg) => {
    ...
    if (msg.is_generating) {
      ...
      const savedText = (savedBotIdx > 0 && savedBotIdx < chatMessages.value.length)
      ...
      if (savedBotIdx >= 0 && savedBotIdx < chatMessages.value.length) {
        chatMessages.value[savedBotIdx] = { type: 'bot', contentType: 'text', text: '' };
      }
      ...
      try {
        ...
        if (hasLoadedOlderPages.value) {
          ...
          const olderCount = Math.max(0, chatMessages.value.length - freshHistory.length);
          const merged = [...chatMessages.value.slice(0, olderCount), ...freshHistory];
...
onBeforeUnmount(() => {
  ...
  for (const type of [
    ...
    'image_progress', 'image_done', 'image_error', 'connection_lost', 'connection_ack',
  ]) {
